...
const getAll = async (req, res, next) => {
    try {
      ...
      if (allComments.length > 0) {
        ...
        return res.status(404).json("No comments in the database.");
      }
    } catch (error) {
      return (
        res.status(500).json({
  ...
  const updateComment = async (req, res) => {
    try {
      ...
      try {
        ...
        if (commentById) {
          const customBody = {
            textComment: req.body?.textComment ? req.body.textComment : commentById.textComment,
          ...
          if (comment) {
            try {
              await User.updateMany(
                { sentComments: idComment, receivedComments: idComment, likedComments: idComment },
                {
                  $pull: {
              ...
              try {
                await Post.updateMany({ comments: idComment }, { $pull: { comments: idComment } });
